...
2. 左下角头像 → **Settings（设置）** → **账户安全与登录（Account & security）** → 打开 **开发者模式（Developer mode）**
3. 返回设置 → **插件（Plugins）** → 点 **创建连接器**：
   - 连接器名称：`禅道`
   - MCP 服务器 URL：粘贴第 1 步复制的那条地址
   - 身份验证：选 **无身份验证**
   - 点 创建
4. 回到聊天输入框 → 点插件/工具图标 → 勾选「**禅道**」
...
按顺序查：①开发者模式开了吗（设置 → 账户安全与登录）②URL 粘贴完整吗（以 `/mcp/` 开头一长串）③到注册页重新注册拿新 URL 换上。
